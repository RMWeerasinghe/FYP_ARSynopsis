...
from gusum import gusum_extracter
...
def gusum_summarizer(clustering:dict[int:list[Sentence]],p_to_select: float) -> dict[int:list[Sentence]]:
    """
    Performs GUSUM summarization using TextRank algorithm on a given clustering solution.
    ...
    extractive_summary = dict()
    for label, cluster in clustering.items():
        extracted_sentences = gusum_extracter(cluster,p_to_select)
        extractive_summary[label] = extracted_sentences
        total_doc_length += len(cluster)
        summary_length += len(extracted_sentences)
    end = time.time()
    print(f"Extracted {summary_length} sentences from {total_doc_length} belongs to {len(clustering)} clusters in {round((end-start),2)} s using GUSUM summarizer.\nAverage summary length: {summary_length/len(clustering)}")
    return extractive_summary
