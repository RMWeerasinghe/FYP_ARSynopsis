...
const PdfViewer = ({
  ...
  const onDocumentLoadSuccess = ({ numPages }) => {
    ...
    console.log("Number of pages loaded:", numPages);
  };
  ...
  useEffect(() => {
    console.log("Input Sentence : ", inputSentenceArray);
    setResultArray([]); // Clear previous results
    setSentenceArrray([]);
    if (
      inputSentenceArray.length > 0 &&
      response &&
      ...
      response.sentences.forEach((pageData) => {
        ...
        inputSentenceArray.forEach((inputSentence) => {
          sentences.forEach((sentence, index) => {
            const inputLower = inputSentence.toLowerCase();
            const sentenceLower = sentence.toLowerCase();
            
            console.log("Input Sentence Lower:", inputLower);
            // console.log("Arrived for Sen Processing");
            

            // Substring check
            const isSubstringMatch = sentenceLower.includes(inputLower);
            const isSubstringMatchInput = inputLower.includes(sentenceLower);
            ...
            if (
              isSubstringMatch ||
              isSubstringMatchInput ||
              similarity >= 0.7
            ) {
              // Found an approximate match, extract surrounding sentences
              console.log("Found a match:", sentenceLower);
              const surrounding = {
              ...
              console.log("Surrounding Sentence:", surrounding);
              ...
              setSentenceArrray((prevResults) => {
                const exists = prevResults.some(
                  (item) =>
                    item.page === surrounding.page &&
                    item.current === surrounding.current
                );
                if (exists) return prevResults;
                return [...prevResults, surrounding];
              });
  ...
  }, [inputSentenceArray, response]);

  return (
    <div className="pdf-container">
      {loading ? (
        <>
          <div style={{ height: 100, padding: 249 }}>Loading</div>
        </>
      ) : (
